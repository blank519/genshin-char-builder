...
def update():
    urls = {'char':"https://genshin.honeyhunterworld.com/db/char/characters/?",
            'sword':"https://genshin.honeyhunterworld.com/sword/", 
            ...
            'artifact':"https://genshin.honeyhunterworld.com/db/artifact/?"}

    df = pd.read_csv("char_data/chars.csv")
    page = requests.get(urls['char'])
    soup = BeautifulSoup(page.content, 'html5lib')

    #Updates the database of characters based on data from honeyhunterworld
    char_elems = soup.find_all('div', class_='char_sea_cont')
    for char in char_elems:
        #Find character name and element
        ...
        #Checks if the character has already been added to the list of characters by checking the name (covers most cases).
        #To account for the multiple travelers (who all share the same name), element is added to their name.
        if(name not in names):
            print("Adding new character: " + name)

            #Add/update the character's personal data table.
            char_data = update_character_data(name)
            char_df = pd.DataFrame(char_data)
            df_name = 'char_data/' + name + '.csv'
            char_df.to_csv(df_name, index=False, encoding='utf-8') 
            
            #Find the character's weapon type.
            weapon_src = char.find('img', class_='sea_weptype_element')['src']
            weapon = None
            possible_weapons = ['sword','claymore','polearm','bow','catalyst']
            for possible_weapon in possible_weapons:
                if possible_weapon in weapon_src:
                    weapon = possible_weapon.capitalize()
            
            #Find the character's icon. 
            icon_name = name + ".png"
    ...
    for weapon in ['sword','claymore','polearm','bow','catalyst']:
        page = requests.get(urls[weapon])
        type = weapon.capitalize()
        ...
        for weapon in weapons:
            ...
            #Check if the weapon is already registered by checking its name.
    ...
    page = requests.get(urls['artifact'])
    soup = BeautifulSoup(page.content, 'html5lib')
